...
export const api = {
  ...
  getProteinDetails: async (uniprotId: string): Promise<ProteinDetails> => {
    const response = await apiClient.get(`/protein/${uniprotId}/details`);
  ...
  extractYield: async (
    text: string,
  ): Promise<{ yield: YieldInfo | null } | null> => {
    const response = await apiClient.post("/publications/extract-yield", {
      text,
    });
    return response.data;
  },
